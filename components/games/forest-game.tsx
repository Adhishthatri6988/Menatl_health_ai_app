...
import { Trees, Volume2, VolumeX, Play, Pause, Check } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Slider } from "@/components/ui/slider";
import { Progress } from "@/components/ui/progress";

const MEDITATION_DURATION = 3 * 60; // 3 minutes in seconds
...
export function ForestGame() {
  ...
  // Use a ref to hold the audio elements to prevent re-creation on re-renders
  ...
  useEffect(() => {
    // Update volume whenever the slider changes
    Object.values(audioElements.current).forEach((audio) => {
    ...
    return (
       <div className="flex flex-col items-center justify-center h-[400px] space-y-6 bg-background rounded-xl p-6">
        ...
        <p className="text-muted-foreground text-center max-w-sm">
          You've finished your walk through the Grove.
        </p>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[400px] space-y-8 bg-background rounded-xl p-6">
      <div className="relative w-48 h-48">
        <motion.div
          animate={{ scale: isPlaying ? [1, 1.05, 1] : 1, rotate: isPlaying ? [0, 0.5, -0.5, 0] : 0 }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
          className="absolute inset-0"
        >
          <div className="absolute inset-0 bg-gradient-to-b from-emerald-500/10 to-transparent rounded-full blur-xl" />
      ...
      <div className="w-64 space-y-6">
        ...
        <Progress value={progress} className="h-2 bg-primary/10 [&>div]:bg-emerald-400" />
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-mono text-muted-foreground">{formatTime(MEDITATION_DURATION)}</span>
        </div>
      </div>
    </div>
  );
